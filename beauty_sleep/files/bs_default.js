(function (window, document, $, miniShop2Config) {
    ...
    miniShop2.Order = {
        ...
        updateDeliveries: function (deliveries, payments) {
            ...
            if (deliveries.length > 0) {
                for (var i in deliveries) {
                    if (deliveries.hasOwnProperty(i)) {
                        var current = $deliveryInputs.filter(miniShop2.Order.deliveryInputUniquePrefix + deliveries[i]);
                        var data_payments = payments.length > 0 ? payments : current.data('payments');
                        current.attr({'disabled': false, 'data-payments': data_payments}).prop('disabled', false).data('payments', data_payments).closest(miniShop2.Order.inputParent).show();
                    }
                }
            }
        ...
        getcost: function () {
            ...
            callbacks.getcost.response.success = function (response) {
                ...
                var delivery_cost = parseInt($(miniShop2.Order.deliveryInput + ':checked').parent().find('.checkbox__descr_price').html());
                $(miniShop2.Order.orderCost).text(miniShop2.Utils.formatPrice(response.data['cost'] + (delivery_cost || 0)));
