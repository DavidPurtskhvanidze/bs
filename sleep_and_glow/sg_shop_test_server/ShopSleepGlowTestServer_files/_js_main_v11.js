    ...
    $(".general-swiper").length > 0
        ...
        $(document).on("click", ".pp_", function (e) {
            ...
            if (($("html,body").addClass("noscroll"), $(".pp").removeClass("show"), $('.pp[data-pp="' + s + '"]').addClass("show"), "promo-video" === s)) t && t.play();
                ...
                var sizeList = $('.pp__size_list');

                // Очищаем блок перед добавлением элементов
                sizeList.empty();

                // Получаем все input с name="options[size]" внутри найденной формы
                var options = form.find('input[name="options[size]"]');

                // Проходим по каждому input и добавляем HTML-код в блок pp__size_list
                options.each(function() {
                    var id = $(this).attr('id');
                    var value = $(this).val();
                    // Генерируем HTML-код для каждого элемента и добавляем в pp__size_list
                    sizeList.append(
                        '<div class="pp__size_list-item">' +
                        '<div class="pp__size_list-item-uni" id="' + id + '">' +
                        value +
                        '</div>' +
                        '</div>'
                    );
                ...
                $(".pp__size_list-item-uni").click(function() {
                    form.find('input[name="options[size]"]').prop('checked', false).attr('checked', false);
                    var size = form.find('.b-mod-size#'+$(this).attr("id"));
                    console.log(size);
                    size.addClass("checked");

                    size.prop('checked', true).attr('checked', 'checked').trigger('change');
                    size.click();
                    form.submit();
                    $(".pp__size").removeClass("show");
                })
